...
> ## ✨ 全网独家自动签到功能
>
> 课堂签到自动监听 + 手势/二维码自动签 + 补签改签，**想直接体验成品请到线上版本**：<https://study.atvkh.xyz/>
> （本仓库是命令行/源码版，线上版还含可视化控制台、账号托管、自动答题等完整能力）
...
| 功能 | 说明 |
|------|------|
| 一键刷课 | SPOC/MOOC/资源库三类课程进度秒刷(固定快速模式，无需选择) |
...
**流程(回车后约 1~1.5 秒,实测中位 ~1.0 秒)**:
1. 选择登录新账号时,后台立即预热:隐藏的 Chromium(窗口在屏幕外且不入任务栏/Alt-Tab,全程无可见痕迹)加载内置极简登录页并预先弹出滑块,同时完成鼠标热身——与你在终端输账密并行,零等待;终端焦点被抢时会自动抢回,无需重新点击面板
2. 终端输入账号密码回车:账密注入 → 双证据融合(白帽变换 + Canny 边缘)识别缺口 → 真人形态轨迹拖拽(13-18ms 帧间隔 + 闭环校正)过阿里云滑块 → 拦截登录响应拿 SSO Token;拖距映射支持在线自校准,自动适应滑块参数漂移
3. SSO Token 自动换取 Bearer Token → 拉取用户信息 → 账号持久化到 `accounts.json`

### 3. 失败自动降级链(无需干预)

```
自建极简登录页(2 次尝试,失败按判定分档退避:位置错 0.8s / 行为风控 2.5s 防高频)
    ↓ 全失败(如页面改版/场景漂移)
...
1. 选择 `[1] 登录`，输入账号密码后自动过滑块登录（回车后约 1~1.5 秒；自动登录失败会自动重试并转人工兜底，详见[登录流程](#登录流程)）
...
| MOOC | 6个API探测 + 跨课件车道并发 | 服务器每次+5秒；**申报时长直接取平台树里的 `videoTime`**(读不到才用解析值/按类型定值)，条数=时长÷5；单课件内部严格串行、课件之间 12 路并行，平台拒绝(如非开课时间)时直接报出原因；收尾用平台已完成集合复核并补满 |
| 资源库 | 明文 JSON + 申报式递增流 | 平台只在 `actualNum` 严格增大时写回，故每格最多 2 跳、已记满的课件免发；`.swf` 动画走计数形单次上报；URL 末尾斜杠必需 |

**实测耗时(单账号、家用宽带出口)**

| 场景 | 耗时 |
|------|------|
| 资源库课件树扫描(1004 个课件 / 1191 次请求，按层 16 路并发) | ≈4 秒 |
| 资源库一门未刷过的课(395 个课件)刷到平台 100% | 152 秒，实发 790 条心跳 |
| 同一门课再点一次(全部已记满) | 19 秒，零心跳上报 |
| MOOC 一门课 188 个课件(真实时长档) | 89 秒 / 被平台接受 4296 条，平台复核 188/188 已学完 |
| 同一门课旧口径(每格虚增 20~40 分钟) | ≈13 分钟 / ≈3.7 万条心跳 |
| MOOC 12 门课 2138 格的媒体头解析任务数 | 899 次 → 54 次(有 `videoTime` 的格不再解析) |
